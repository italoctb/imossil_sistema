				...
				'<div class="post '.$class_border.'" onmouseenter="show_details(this)" onmouseleave="hide_details(this)" style="background: url(';if($post->img_post){echo base_url("static/uploads/".$post->user_insta."/".$post->img_post);}else{echo base_url("static/images/logo_convence.png");}echo ') center; background-size: cover">
				  <div class="post-details '.$class_bg.'">
					  ...
					  <div style="float: right">
					  	<form action="'.base_url($emp_insta->user_insta.'/change_status_cliente').'" method="post">
					  	  <input type="hidden" name="status" id="status'.$i.'">
					  	  <input type="hidden" name="id_post_s" id="id_post_s" value="'.$post->id_post_cliente.'">
						  <input type="hidden" name="user_change" value="'.$user_data->usuario.'">
					  	  ';if(($user_data->permissao == 3 && $user_data->id_emp == $emp_insta->id_emp) || $user_data->permissao == 0){
					  ...
					  <div style="min-height: 375px">
						  <h1 style="margin-right: 15px;" id="id_post">Post_id: #'.substr($post->id_post_cliente,2).'</h1><a href="'.$post->link_briefing.'"><i class="fa fa-link fa-lg" aria-hidden="true"></i></a>
						  ...
						  <div style="display: inline-block; min-width: 172px">
							  <ul>';
							foreach ($rs as $r){
								if($r->id_post_cliente == $post->id_post_cliente){
									if($r->nome != 'facebook'){
							  ...
							  </div>';if(($user_data->permissao == 3 && $user_data->id_emp == $emp_insta->id_emp) || $user_data->permissao == 0){
									echo '<a href="'.base_url($emp_insta->user_insta.'/'.substr($post->id_post_cliente,2).'/edit_post_cliente').'" style="float: right"><button class="follow_button">Editar Post</button></a>';
							  }echo'
